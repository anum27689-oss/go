  ...
  return (
    <div className="bg-surface text-on-surface font-body min-h-screen flex flex-col">
        ...
        <main className="flex-grow pt-24 pb-28 px-6 flex flex-col items-center justify-center max-w-lg mx-auto w-full">
            ...
            <div className="relative w-full flex flex-col items-center">
                ...
                <div className="mb-12 text-center">
                    ...
                    <div className="text-on-surface-variant font-body text-sm tracking-[0.2em] uppercase opacity-60">{t('tasbeeh.count')}</div>
                </div>
                ...
                <div className="relative group">
                    ...
                    <button onClick={increment} className="relative w-64 h-64 rounded-full bg-gradient-to-br from-primary to-primary-container text-on-primary flex flex-col items-center justify-center tasbeeh-glow active:scale-95 transition-all duration-200 group">
                        ...
                        <span className="font-headline font-bold text-lg tracking-wide">{t('tasbeeh.tap')}</span>
                ...
                <div className="mt-16 flex flex-col items-center gap-6">
                    ...
                    <p className="text-on-surface-variant text-xs font-medium tracking-wide">{t('tasbeeh.goal').replace('{progress}', String(goalProgress)).replace('{dailyGoal}', String(dailyGoal))}</p>
                </div>
            </div>
            ...
            {showHistory && (
              <section className="grid grid-cols-2 gap-4 w-full mt-16">
                  <div className="bg-surface-container-low p-5 rounded-lg border border-outline-variant/15">
                      ...
                      <p className="text-on-surface-variant text-xs font-medium mb-1">{t('tasbeeh.lastSession')}</p>
                      <p className="text-on-surface font-headline font-bold text-lg">{t('tasbeeh.taps').replace('{count}', String(lastSessionTaps))}</p>
                  </div>
                  <div className="bg-surface-container-low p-5 rounded-lg border border-outline-variant/15">
                      <span className="material-symbols-outlined text-secondary mb-3" style={{fontVariationSettings: "'FILL' 1"}}>stars</span>
                      <p className="text-on-surface-variant text-xs font-medium mb-1">{t('tasbeeh.streak')}</p>
                      <p className="text-on-surface font-headline font-bold text-lg">{t('tasbeeh.days').replace('{count}', String(streak))}</p>
                  </div>
              </section>
            )}
        </main>
        ...
        <nav className="fixed bottom-0 w-full z-50 bg-surface/80 backdrop-blur-2xl flex justify-around items-center h-20 px-2 max-w-2xl mx-auto left-0 right-0 pb-[env(safe-area-inset-bottom)]">
            ...
            <Link href="/tasbeeh" className="flex flex-col items-center justify-center gap-1 bg-primary text-on-primary rounded-xl p-2 h-14 w-16 transition-all duration-200 active:scale-90 tap-highlight-none">
                <span className="material-symbols-outlined" style={{fontVariationSettings: "'FILL' 1"}}>adjust</span>
                <span className="text-xs font-bold">{t('nav.tasbeeh')}</span>
            </Link>
